...
CopyInstruction &CopyPlan::join(CopyInstruction &i1, CopyInstruction &i2, int maxGap)
{
    ...
    if (i1.range < i2.range)
        ifirst = &i1, ilast = &i2;
    else if (i2.range < i1.range)
        ifirst = &i2, ilast = &i1;

    if (ifirst && ilast) {
        bool gapTooLarge = (ifirst->range.end + maxGap + 1 < ilast->range.start);
        if (!freezeCNums && trySwapRanges(ilast->range, ifirst->range.end + 1))
            gapTooLarge = false;
        if (gapTooLarge)
            return invalidInsn;
    }

    i1.range.start = std::min(i1.range.start, i2.range.start);
    i1.range.end = std::max(i1.range.end, i2.range.end);
    i2.invalidate();

    return i1;
}

// Try to swap range so that the range [min0, max0] is moved to start at min1.
...
void CopyPlan::legalizeSIMD(bool initial)
{
    ...
    for (size_t n = 0; n < ninsn; ) {
        ...
        if (simd0 < minSimd0 && minSimd0 < i.simd) {
            rerun = true;
            simd0 = minSimd0;
        }
        ...
        if (simd0 < i.simd || splitting) {
            ...
            isplit.simd = simd0;
            isplit.range.end = isplit.range.start + simd0 - 1;
...
void CopyPlan::optimizeWriteCombine()
{
    ...
    for (size_t n = 0; n + 1 < ninsn; ) {
        ...
        for (n1 = n + 1; n1 < ninsn; n1++) {
            ...
            range.start = std::min(range.start, i1.range.start);
            range.end = std::min(range.end, i1.range.end);
        }
...
void CopyPlan::materializeTemps(const GRFAllocator &grfAllocator, const FlagAllocator &flagAllocator)
{
    ...
    auto groupInstructions = [&](std::vector<bool> &joined, CopyRange &range) {
        ...
        joined.assign(range.end - range.start + 1, false);

        for (auto &i: insns)
            if (i.phase >= minPhaseTemp && i.phase <= maxPhaseTemp)
                for (auto j = i.range.start; j <= i.range.end; j++)
                    joined[j - range.start] = true;
    };
    ...
    const CopyRange range0 = range;

    /* Sort instructions and temporaries by parent instruction (cnum) */

    auto cmp = [&](int i, int j) { return temps[i].range < temps[j].range; };
    ...
    while (order != end) {
        for (; order != end; ++order) {
            ...
            if (!manager.allocate(temp)) {
                range.end = temp.range.start; break;
            }
        }

        /* Back off to the nearest instruction group boundary */
        while (range.end > 0 && joined[range.end - 1])
            range.end--;
...
void CopyPlan::dump(std::ostream &os, bool sortInfo, int n) const
{
    if (n < 0) n = (int)insns.size();
    n = std::min(n, (int)insns.size());
    for (int i = 0; i < n; ++i) {
