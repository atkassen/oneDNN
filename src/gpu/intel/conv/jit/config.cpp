...
walk_order_t maybe_fixup_group_with_small_channels(
    ...
    walk_order_t fixed;
    fixed.add(pvars::g, static_cast<int>(grid_tile.at(pvars::g)), 0);
    for (auto &b : walk_order.blocks()) {
        if (b.dim == pvars::g) continue;
        fixed.add(b.dim, static_cast<int>(b.size), b.grid_id);
    }
...
walk_order_t maybe_fixup_large_weights(
    ...
    if (grid_tile.has(pvars::oh))
        fixed.add(pvars::oh, static_cast<int>(grid_tile.get(pvars::oh)), 0);
    for (auto &b : walk_order.blocks()) {
...
walk_order_t get_default_walk_order(
    ...
    for (int i = 0; i < 3; i++) {
        for (auto &d : grid_dims[i]) {
            if (grid_tile.has(d))
                walk_order.add(d, static_cast<int>(grid_tile[d]), i);
        }
    }
...
public:
    ...
    entry_t next(const tile_t &inner) {
        ...
        for (auto &d : inner) {
            ...
            if (bmnk == pvars::m) {
                m_size *= static_cast<int>(inner[d]);
            } else if (bmnk == pvars::n) {
                n_size *= static_cast<int>(inner[d]);
            }
        }
            ...
            switch (step) {
                case 0:
                    if (grid_inner.has(b.dim)) {
                        walk_order.add(
                                b.dim, static_cast<int>(grid_inner[b.dim]), 0);
                    }
                    break;
                case 1:
                case 2:
                    ...
                    walk_order.add(b.dim, static_cast<int>(rem), 0);
                    break;
            }
        }
    }
