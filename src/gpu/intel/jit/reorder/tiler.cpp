...
std::vector<tensor_t> tiles(
    ...
    std::sort(candidate_tiles.begin(), candidate_tiles.end(),
            [&](const tensor_t &l, const tensor_t &r) {
                ...
                auto l_msg_load
                        = message_latency(hw, a, l) + message_latency(hw, b, l);
                auto r_msg_load
                        = message_latency(hw, a, r) + message_latency(hw, b, r);

                // Choose tiles with less message overhead per thread
                if (l_eu_util * l_msg_load != r_eu_util * r_msg_load)
                    return (r_eu_util * r_msg_load < l_eu_util * l_msg_load);
