...
private:
    ...
    void init_dense_blocks(const constraint_set_t &cset) {
        ...
        dense_block_size_ = static_cast<int>(
                tile.elems() * type().size() / type().packing());
        ...
        view_.for_each_tile(tile, [&](const icoord_t &start) {
            ...
            int f = static_cast<int>(get_max_const_factor(off, cset));
...
private:
    ...
    int remaining_elems() const {
        return static_cast<int>(layout_.elems() - elems_);
    }
...
bool access_builder_t::try_build_2d(send_params_t &send_params) {
    ...
    if (use_virtual_surface) {
        ...
        surface_width = static_cast<int>(b0.size);
        surface_height = static_cast<int>(b1.size);
    } else {
        surface_width = static_cast<int>(tlayout.elems(w_dim_idx));
        surface_height = static_cast<int>(tlayout.elems(h_dim_idx));
...
send_2d_hint_t get_send_2d_hint(const dsl::kernel::options_t &options,
    ...
    return get_send_2d_hint(send_op, view.type(), false, false,
            static_cast<int>(b0.size), static_cast<int>(b1.size));
}
...
send_2d_hint_t get_send_2d_hint(const dsl::kernel::options_t &options,
    ...
    if (send_op == send_op_t::load && fma_kind == fma_kind_t::dpas
        ...
        hint = get_send_2d_hint(send_op, view.type(), vnni, transpose,
                static_cast<int>(b0.size), static_cast<int>(b1.size), b0_blk,
                b1_blk);
    } else {
        if (b0.size >= 128) return hint;
